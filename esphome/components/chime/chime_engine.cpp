...
#include <cstring>
  ...
  void ChimeEngine::compute_local_background_() {
    ...
    for (uint32_t i = 0; i < nf; ++i) {
      ...
      for (uint32_t j = 0; j < nf; ++j) {
        if (i == j)
          continue;
        if (std::fabs(effective_freqs_[i] - effective_freqs_[j]) < det_cfg_.guard_separation_hz)
          continue;
  ...
  bool ChimeEngine::try_emit_tick(uint32_t now_ms, std::vector<Event> & out_events, TickSnapshot * out_tick) {
    ...
    if (sample_rate_hz_ > 0)
      samples_in_tick = static_cast<uint32_t>(sample_rate_hz_ * det_cfg_.tick_interval_ms / 1000.0f);
    bool full = (samples_in_tick > 0 && tick_sample_count_ >= samples_in_tick) || (frame_count_ >= MAX_FRAMES_PER_TICK);
    if (!full)
      return false;
    ...
    for (size_t ci = 0; ci < chimes_.size(); ++ci) {
      ...
      c.evaluate_pattern(now_ms, spectrum_db_.data(), eval_floor, local_bg_db_.data(), eval_onset,
                         static_cast<uint32_t>(ci), tick_idx, [&](const Event &ev) { out_events.push_back(ev); });
    }
