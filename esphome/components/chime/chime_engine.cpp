...
void ChimeEngine::set_detector_config(const DetectorConfig &cfg) {
  det_cfg_ = cfg;
  for (auto &c : chimes_)
    c.set_tick_interval_ms(det_cfg_.tick_interval_ms);
...
void ChimeEngine::add_chime(const ChimeConfig &cfg) {
  ChimePattern p(cfg);
  p.set_tick_interval_ms(det_cfg_.tick_interval_ms);
...
void ChimeEngine::feed_pcm(const int16_t *samples, size_t count) {
  ...
  while (pos < count) {
    ...
    uint32_t take =
        std::min<uint32_t>(need, static_cast<uint32_t>(count - pos));
    std::memcpy(frame_buf_.data() + frame_buf_offset_, samples + pos,
                take * sizeof(int16_t));
...
void ChimeEngine::compute_local_background_() {
  ...
  for (uint32_t i = 0; i < nf; ++i) {
    ...
    for (uint32_t j = 0; j < nf; ++j) {
      if (i == j)
        continue;
      if (std::fabs(effective_freqs_[i] - effective_freqs_[j]) <
          det_cfg_.guard_separation_hz)
        continue;
      if (spectrum_db_[j] < min_db)
        min_db = spectrum_db_[j];
    }
    if (min_db > 299.0f)
      min_db = *std::min_element(spectrum_db_.begin(), spectrum_db_.end());
...
uint32_t ChimeEngine::frames_per_tick_() const {
  ...
  if (fpt < 1)
    fpt = 1;
  if (fpt > MAX_FRAMES_PER_TICK)
    fpt = MAX_FRAMES_PER_TICK;
  return fpt;
}

float ChimeEngine::tick_audio_ms_() const {
  if (sample_rate_hz_ <= 0)
    return (float)det_cfg_.tick_interval_ms;
  return (frames_per_tick_() * det_cfg_.window_size) / sample_rate_hz_ *
         1000.0f;
}

bool ChimeEngine::try_emit_tick(uint32_t now_ms, std::vector<Event> &out_events,
  ...
  if (sample_rate_hz_ > 0)
    samples_in_tick = static_cast<uint32_t>(
        sample_rate_hz_ * det_cfg_.tick_interval_ms / 1000.0f);
  ...
  for (uint32_t t = 0; t < total_filters_; ++t) {
    ...
    if (onset_count_[t] < ONSET_LOOKBACK)
      onset_count_[t]++;
  }
  ...
  for (uint32_t t = 0; t < total_filters_; ++t) {
    ...
    for (uint32_t i = 0; i < nvalid; ++i) {
      ...
      if (onset_history_[t * ONSET_LOOKBACK + idx] < min_db)
        min_db = onset_history_[t * ONSET_LOOKBACK + idx];
    }
    if (min_db > 299.0f)
      min_db = spectrum_db_[t];
  ...
  if (!noise_floor_ready_) {
    ...
    for (uint32_t t = 0; t < total_filters_; ++t) {
      ...
      noise_floor_[t] = cur < fl
                            ? fl + det_cfg_.noise_floor_alpha_down * (cur - fl)
  ...
  for (size_t ci = 0; ci < chimes_.size(); ++ci) {
    auto &c = chimes_[ci];
    const float *eval_onset = (c.config().onset_contrast_db > 0.0f)
                                  ? onset_contrast_.data()
                                  : nullptr;
    c.evaluate_pattern(now_ms, spectrum_db_.data(), eval_floor,
                       local_bg_db_.data(), eval_onset,
  ...
  if (out_tick != nullptr) {
    ...
    for (size_t ci = 0; ci < chimes_.size(); ++ci) {
      ...
      uint32_t step = c.pattern_active
                          ? std::min<uint32_t>(c.match_index, num_steps - 1)
                          : 0;
      d.step = step;
      uint32_t t_ms = (step < c.config().pattern_times_ms.size())
                          ? c.config().pattern_times_ms[step]
                          : NO_TIME;
      ...
      for (size_t i = 0; i < indices.size(); ++i) {
        ...
        auto g = c.bin_gates(spectrum_db_.data(), noise_floor_.data(),
                             noise_floor_ready_, local_bg_db_.data(),
...
RunResult ChimeEngine::run_offline(const float *mono, size_t num_samples,
                                   float fs) {
  RunResult res;
  // Save/restore streaming state by working in a fresh engine copy logic
  // inlined here Build freq map if not yet built (allow caller to add_chime
  // first)
  if (total_filters_ == 0 && !chimes_.empty())
    build_frequency_map();
  set_sample_rate(fs);
  ...
  uint32_t total_ticks =
      (fpt * n > 0) ? static_cast<uint32_t>(num_samples / (fpt * n)) : 0;
  ...
  for (uint32_t tick_idx = 0; tick_idx < total_ticks; ++tick_idx) {
    ...
    // But we already computed accum, so bypass feed and emit inline via
    // try_emit's spectrum logic: To avoid double accum, we set accum already,
    // so call a helper emit directly: Instead call try_emit_tick which will do
    // spectrum conversion and state machine. It expects accum already averaged
    // with inv, so we leave accum as accumulated power and let try_emit
    // average. We already filled accum, so just set frame_count and call.
    std::vector<Event> tick_events;
    TickSnapshot snap;
    // Adjust: try_emit will average accum*inv; we already have accum as sum, so
    // fine.
  ...
  // Build sensor intervals like js (not needed for C++ caller but useful for
  // wasm) Caller can derive from events.
