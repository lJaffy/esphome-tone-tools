...
struct Chime {
  ...
  bool chord_present_(const float *spectrum_db, const float *noise_floor,
                      const float *local_bg, const bool *coherent, uint8_t step,
                      float &peak_db) const;
...
protected:
  ...
  float *prev_phase_{nullptr};
  float *expected_dphi_{nullptr};
  float *coherence_err_{nullptr};
  uint8_t *coherence_count_{nullptr};
  uint8_t *have_prev_phase_{nullptr};
  bool *coherent_{nullptr};
  uint32_t coherence_ring_pos_{0};
