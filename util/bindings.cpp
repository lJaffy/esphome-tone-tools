// util/bindings.cpp — Emscripten embind for chime core (moved from
// esphome/components/chime/wasm). Built with emsdk 3.1.6: emcc
// ../esphome/components/chime/chime_pattern.cpp
// ../esphome/components/chime/chime_engine.cpp bindings.cpp --bind -o
// ../wasm/chime.js Also compiles as plain C++ (no-op) for syntax checking
// without emcc.

#include "../esphome/components/chime/chime_engine.h"
#include "../esphome/components/chime/chime_types.h"
  ...
  if (js.hasOwnProperty("pattern")) {
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      for (size_t j = 0; j < m; ++j)
        c.pattern[i][j] = chord[j].as<float>();
    }
  }
  if (js.hasOwnProperty("times")) {
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      if (v.isNull() || v.isUndefined())
        c.pattern_times_ms[i] = NO_TIME;
      else
        c.pattern_times_ms[i] = (uint32_t)(v.as<double>() * 1000.0);
    }
    ...
    c.pattern_times_ms.resize(n);
    for (size_t i = 0; i < n; ++i)
      c.pattern_times_ms[i] = times[i].as<uint32_t>();
  }
  if (js.hasOwnProperty("minDurationMs"))
    c.min_duration_ms = js["minDurationMs"].as<uint32_t>();
  if (js.hasOwnProperty("maxDurationMs"))
    c.max_duration_ms = js["maxDurationMs"].as<uint32_t>();
  if (js.hasOwnProperty("thresholdDb"))
    c.threshold_db = js["thresholdDb"].as<float>();
  if (js.hasOwnProperty("snrMarginDb"))
    c.snr_margin_db = js["snrMarginDb"].as<float>();
  if (js.hasOwnProperty("prominenceDb"))
    c.prominence_db = js["prominenceDb"].as<float>();
  if (js.hasOwnProperty("onsetContrastDb"))
    c.onset_contrast_db = js["onsetContrastDb"].as<float>();
  if (js.hasOwnProperty("tailGraceMs"))
    c.tail_grace_ms = js["tailGraceMs"].as<uint32_t>();
...
static DetectorConfig jsToDetectorConfig(const val &js) {
  ...
  if (!js.isNull() && !js.isUndefined()) {
    if (js.hasOwnProperty("windowSize"))
      d.window_size = js["windowSize"].as<uint16_t>();
    if (js.hasOwnProperty("tickIntervalMs"))
      d.tick_interval_ms = js["tickIntervalMs"].as<uint32_t>();
    if (js.hasOwnProperty("guardSeparationHz"))
      d.guard_separation_hz = js["guardSeparationHz"].as<float>();
    if (js.hasOwnProperty("noiseFloorAlphaDown"))
      d.noise_floor_alpha_down = js["noiseFloorAlphaDown"].as<float>();
    if (js.hasOwnProperty("noiseFloorAlphaUp"))
      d.noise_floor_alpha_up = js["noiseFloorAlphaUp"].as<float>();
    // legacy aliases
    if (js.hasOwnProperty("window_size"))
      d.window_size = js["window_size"].as<uint16_t>();
    if (js.hasOwnProperty("tick_interval_ms"))
      d.tick_interval_ms = js["tick_interval_ms"].as<uint32_t>();
  }
  return d;
}
...
static val eventToJs(const Event &e) {
  ...
  switch (e.type) {
  case EventType::PatternStart:
    typeStr = "pattern_start";
    break;
  case EventType::FallingEdge:
    typeStr = "falling_edge";
    break;
  case EventType::ChordMatch:
    typeStr = "chord_match";
    break;
  case EventType::Detected:
    typeStr = "detected";
    break;
  case EventType::MinDurationDiscount:
    typeStr = "min_duration_discount";
    break;
  case EventType::StepTimeout:
    typeStr = "step_timeout";
    break;
  case EventType::MaxDurationTimeout:
    typeStr = "max_duration_timeout";
    break;
...
public:
  ...
  val run() {
    ...
    val freqs = val::array();
    for (size_t i = 0; i < r.meta.freqs.size(); ++i)
      freqs.set(i, val(r.meta.freqs[i]));
    ...
    for (size_t i = 0; i < r.ticks.size(); ++i) {
      ...
      for (size_t ci = 0; ci < dcfg_chimes_.size(); ++ci) {
        // Derive from engine state? For offline we don't have per-chime
        // snapshot separate; approximate from diag
        val cj = val::object();
